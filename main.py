...
def find_local_image(image_field: str, images_dir: str) -> str:
    ...
    1. Local Storage URLs, e.g. '/data/local-files/?d=Images/img001.jpg'
       -> the real relative path is in the query string ('d=...'), not the URL path
    2. Uploaded-file URLs, e.g. '/data/upload/3/abc123-img001.jpg'
       -> basename is 'abc123-img001.jpg', often with a hash prefix before '-'
    3. A plain filename or relative path already
    """
    parsed = urlparse(image_field)
    query_params = parse_qs(parsed.query)
 
    # Case 1: local-files storage -> filename lives in the 'd' query param
    if "d" in query_params:
        rel_path = unquote(query_params["d"][0])
        local_path = os.path.join(images_dir, os.path.basename(rel_path))
        if os.path.exists(local_path):
            return local_path
        # Also try the full relative path as-is (in case images_dir is the parent
        # of the folder structure referenced in 'd', e.g. 'Images/img001.jpg')
        local_path_full = os.path.join(os.path.dirname(images_dir.rstrip("/")), rel_path)
        if os.path.exists(local_path_full):
            return local_path_full
 
    # Case 2 / 3: use the basename of the URL path
    filename = os.path.basename(unquote(parsed.path))
    local_path = os.path.join(images_dir, filename)
    if os.path.exists(local_path):
        return local_path
 
    # Fallback: strip a Label-Studio upload hash prefix like "abc123-image.jpg"
    if "-" in filename:
